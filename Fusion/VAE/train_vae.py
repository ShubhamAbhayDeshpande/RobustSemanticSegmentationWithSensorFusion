...
from DataLoaderForest import ForestDataset_RGB, ForestDataset_RGB_NIR
import wandb
...
parser.add_argument(
    "--model_name",
    "-mn",
    default="Uni_dataset_rgb_nir_512x512",
...
parser.add_argument(
    "--dataset_root",
    "-dr",
    default="/home/deshpand/noadsm/datasets/Uni-dataset/001/rgb",
...
parser.add_argument(
    "--nir_dataset",
    "-nir",
    default="/home/deshpand/noadsm/datasets/Uni-dataset/001/nir",
    help="NIR images root folder.",
    type=str,
    required=False,
)
parser.add_argument(
    ...
    "-m",
    default="RGB",
    help="Based on this, convert the RGB image Gray scale or convert gray scale to RGB.",
    type=str,
    required=False,
)
parser.add_argument(
...
experiment = wandb.init(
    project="ResNet_VAE_512x512",
    resume="allow",
    name=args.model_name + str(args.image_size),
    config={},
)
...
data_set = ForestDataset_RGB(folder_path=args.dataset_root, transforms=transform)
...
test_images = next(dataiter)  # dataiter.next()
...
# Create the feature loss module if required (Feature loss)
...
for epoch in trange(start_epoch, args.nepoch, leave=False):
    ...
    for i, (images) in enumerate(tqdm(train_loader, leave=False)):
        ...
        mu_lst.append(mu.mean().item())
        mu_var_lst.append(mu.var().item())
        log_var_lst.append(log_var.mean().item())
        log_var_var_lst.append(log_var.var().item())
        kl_loss_lst.append(kl_loss.item())
        img_mse_lst.append(feature_loss.item())
        feature_loss_lst.append(feature_loss.item())
        ...
        if (current_iter + 1) % args.save_interval == 0:
            ...
            with torch.no_grad():
                with torch.cuda.amp.autocast():
                    ...
                    recon_img, mu, log_var = vae_net(test_images["img"].to(device))
                    ...
                    vutils.save_image(
                        img_cat,
                        "%s/%s/%s_%d_test.png"
                        % (args.save_dir, "Results", args.model_name, args.image_size),
                        normalize=True,
                    )
                ...
                if os.path.isfile(args.save_dir + "/Models/" + save_file_name + ".pt"):
                    shutil.copyfile(
                        src=args.save_dir + "/Models/" + save_file_name + ".pt",
                ...
                vae_net.train()
    # Logging the above parameters in Wandb for further analysis
    experiment.log(
        {
            "epoch": epoch,
            "learning_rate": optimizer.param_groups[0]["lr"],
            "mean (Mu)": sum(mu_lst) / len(mu_lst),
            "mu_var": sum(mu_var_lst) / len(mu_var_lst),
            "log_var": sum(log_var_lst) / len(log_var_lst),
            "log_var_var": sum(log_var_var_lst) / len(log_var_var_lst),
            "kl_loss": sum(kl_loss_lst) / len(kl_loss_lst),
            "img_mse": sum(img_mse_lst) / len(img_mse_lst),
            "feature_loss": sum(feature_loss_lst) / len(feature_loss_lst),
        }
    )
