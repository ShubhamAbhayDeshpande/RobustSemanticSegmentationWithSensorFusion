...
def hist_similar(x,y):
    ...
    """
    t_min = torch.min(torch.cat((x, y), 1)).item()  # Find the min value between two images
    t_max = torch.max(torch.cat((x, y), 1)).item()  # Find the max value between two images
    return (torch.norm((torch.histc(x, 255, min=t_min, max=t_max)-torch.histc(y, 255, min=t_min, max=t_max)),2))/255 # Norm values between the vectors of histograms for image 1 and image 2. 
