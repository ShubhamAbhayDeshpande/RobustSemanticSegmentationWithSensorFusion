"""
This train file will execute the model with the pre-trained weights. 
The intension is to check if the model with pre-trained weights will provide better results than the model 
...
dir_checkpoint = Path('./checkpoints/pretrained_freiburg_dataset_train_5_epochs_1_batch_size_1e-5_lr')
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    if args.classes == 1: #if model.n_classes == 1:
...
if __name__ == '__main__':
    ...
    # Use the following if you want to train the model from scratch.
    # Change here to adapt to your data
    # n_channels=3 for RGB images
    # n_classes is the number of probabilities you want to get per pixel
    # model = UNet(n_channels=3, n_classes=args.classes, bilinear=args.bilinear)

    # For model pre-trained on ISBI challange, use the following model.
