"""
Program modified for freiburg forest dataset.

Following notes identify what is different in each iteration of the program.

V_0.1 :: Chanded the weight associated with perceptual loss to 1, learning rate changed to 1e-2, stored recreated images for visualization, changed dataset for nir to nir_gray. 
...
writer = SummaryWriter('./original_train_tensorboard_log_V_0.1')

parser.add_argument('--device', default='cuda', required=False, help="Device for running the mdel. Either 'cuda' or 'cpu'.")
parser.add_argument('--ir_img', default='./data/ir', required=False, help='Where ir images are stored.')
parser.add_argument('--rgb_img', default='./data/rgb', required=False, help='Where the rgb images are stored.')
...
parser.add_argument("--exp_name", type=str, default="alpha_1000_beta_1000_gamma_1000", required=False, 
                    help="Name of the experiment being performed.")
args = parser.parse_args()

logging.basicConfig(level= logging.INFO, filename='original_train_V_0.1_log.log',filemode='w', format='[DualBranchNetworkTrain::%(asctime)s:]:%(message)s')
# import scipy.io as scio
# import numpy as np
# from matplotlib import pyplot as plt

def save_regenerated_img(torch_tensor, epoch_int, index_int, experiment_name, batch_size):
    """
    ...
    """
    img_path = os.path.join("./reconstructed_img_train", experiment_name)
    os.makedirs(img_path, exist_ok=True)
    img_name = img_path+ '/' + str(epoch_int)+ '_' + str(index_int) + '.png'
    print(f'Saved reconstructed image under location: {img_name}')
    rand_idx_tensor = torch.randint(0, batch_size, (1,))
...
if __name__ == '__main__':
    ...
    save_name = "H_alpha_1000_beta_1000_gamma_1000"
    os.chdir(r'./')
    ...
    # Hyper-parameters (weights) for losses. The following are default values suggested by author.
    alpha = 1000
    beta = 1000
    gamma = 1000
    ...
    loader_TNO = DataLoader(data_TNO, batch_size = batch_size, shuffle=True)  # This will return the batches with batch size for the entire dataset.

    img_num = len(data_TNO)
    print("Load ---- {} pairs of images: TNO:[{}]".format(img_num,len(data_TNO)))

     # Use WandB for logging weight and bias
    experiment = wandb.init(
        # Setting experiment for logging
        project = "DualBrach_Fusion",
    ...
    mkdir(train_path)
    min_loss = 100  # This value will change based on the dataset and imge complexity.
    ...
    loss = torch.zeros(1)  # Pre-defining loss as zero because we may need to updatae the sceduler for first time based on this loss. 
    ...
    for iteration in range(epochs):  # This will load the epochs.
        ...
        scheduler.step(loss.item()) # This will adjust the learining rate in case lr does not change after multiple iterations.
        img_name = "Freiburg_Forest"
        imgs_T = iter(loader_TNO) # It will create an iterable for the number of batches given by the Dataloader class. Here it is represented by loader_TNO variable.
        _loss_per_epoch = []
        
        tqdms = tqdm(range(int(steps))) # tqdm will provide the progress bar for each step.
        for index in tqdms:  # This will load the batches.
            ...
            img_re= model(img) # Send original image to the encoder and decoder model and get reconstructed image back.
            
            if iteration%3 == 0 and index == 5:
                save_regenerated_img(img_re, iteration, index, args.exp_name, args.batch_size)
            ...
            with torch.no_grad():
                x = img.detach()
            features = loss_network(x)  # Loss network refers to VGG net. Features extracted from this network (for real and reconstructed image) are used for Perceptual loss
            ...
            perceptual_loss = perceptual_loss*1000  # Initially multiplied by 1000. No idea what was the need to do that.
            ...
            loss = mse_loss + alpha*grd_loss + beta*std_loss + gamma*perceptual_loss
            ...
            last_time%60,optimizer.param_groups[0]['lr']))
        ...
        # Before saving the model, log each of the losses into WandB
        epoch_grad_loss = sum(grd_loss_all)/len(grd_loss_all)
        epoch_hist_loss = sum(hist_loss_all)/len(hist_loss_all)
        epoch_mse_loss = sum(mse_loss_all)/len(mse_loss_all)
        epoch_perceptual_loss = sum(perceptual_loss_all)/len(perceptual_loss_all)

        wandb.log(
            {
                "Epoch": iteration,
                "Gradient Loss": epoch_grad_loss,
                "Histogram Loss": epoch_hist_loss,
                "MSE Loss": epoch_mse_loss,
                "Perceptual Loss": epoch_perceptual_loss,
