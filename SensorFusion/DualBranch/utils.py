...
class AEDataset(Data.Dataset):
    def __init__(self, root, root2, resize=256, gray=True):
        # self.files = glob(os.path.join(root, '*.png'))+glob(os.path.join(root2, '*.png'))
        file_root = glob(os.path.join(root, "*.png"))
        file_root_2 = glob(os.path.join(root2, "*.png"))
        self.files = file_root + file_root_2
        print(f"files in folder: {self.files}")
        self.resize = resize
        self.gray = gray
...
class Strategy(nn.Module):
    ...
    def forward(self, y1, y2):
        ...
        if self.mode == "l1":
            ...
            kernel = (
                torch.ones(2 * self.window_width + 1, 2 * self.window_width + 1)
                / (2 * self.window_width + 1) ** 2
            )
...
class test_gray(Test):
    ...
    def get_fusion(
        ...
        img_fusion = fusion(
            x1=img1, x2=img2, model=model, mode=mode, window_width=window_width
        )
...
def test(
    ...
    img_name_list = list(
        set(
            [
                img_list[i].split("\\")[-1].split(".")[0].strip(string.digits)
...
def hist_similar(x, y):
    """
    Function for calculating histogram loss between the reconstructed and original image.
    param array x: First image
    param array y: Second image

    """
    t_min = torch.min(
        torch.cat((x, y), 1)
    ).item()  # Find the min value between two images
    t_max = torch.max(
        torch.cat((x, y), 1)
    ).item()  # Find the max value between two images
    return (
        torch.norm(
            (
                torch.histc(x, 255, min=t_min, max=t_max)
                - torch.histc(y, 255, min=t_min, max=t_max)
            ),
            2,
        )
    ) / 255  # Norm values between the vectors of histograms for image 1 and image 2.
