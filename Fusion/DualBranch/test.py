...
parser.add_argument('--device', default='cuda', required=True, help='Device for running the mdel. Either cuda or cpu.')
parser.add_argument('--weights', default='./train_result/H_best.pkl', required=True, help='Training weights for encoder and decoder')
parser.add_argument('--fusion', default='add', required=True, help='Fusion strategy to be used.')
parser.add_argument('--ir_img', default='./Test_ir/', required=True, help='Where ir images are stored.')
parser.add_argument('--rgb_img', default='./Test_vi/FLIR/', required=True, help='Where the rgb images are stored.')
args = parser.parse_args()

if __name__ == '__main__':

    logging.basicConfig(level=logging.INFO, filename='test_log.log', filemode='a', format='[DualBranch::%(asctime)s]::%(message)s')
    logging.info(r'==========================================Starting Testing=====================================================')
    ...
    model = DenseFuseNet().to(args.device) # For any fusion stratergy, we need to perform encoding step on ir and rgb images. That is why we need this.
    ...
    fusename = ['l1','add','channel']  # These are the types of the fusion stratergies that we can use. For more info read litreature.
    ...
    def test(model):
        ...
        for img in img_list_ir:
            ...
            s_time = time.time()
            feature1, feature2 = model.encoder(img_1_normalized,isTest=True),model.encoder(img_2_normalized,isTest=True)
            #for name in fusename:
            if args.fusion =='channel':
                features = channel_fusion(feature1, feature2,is_test=True) # perform channel fusion before giving the output to the decoder.
                out = model.decoder(features).squeeze(0).detach().cpu() # Do decoding on CPU? why? Can we use GPU here instead?
                save_name = 'result/'+args.fusion+'/fusion_'+str(img_name)+'.jpg'
