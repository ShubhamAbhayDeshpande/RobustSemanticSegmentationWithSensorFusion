...
parser.add_argument('--device', default='cpu', required=True, help="Device for running the mdel. Either 'cuda' or 'cpu'.")
parser.add_argument('--ir_img', default='./data/ir', required=True, help='Where ir images are stored.')
parser.add_argument('--rgb_img', default='./data/rgb', required=True, help='Where the rgb images are stored.')
...
logging.basicConfig(level= logging.INFO, filename='normalized_input_our_data_train.log',filemode='w', format='[DualBranchNetworkTrain::%(asctime)s:]:%(message)s')
# import scipy.io as scio
# import numpy as np
...
def save_regenerated_img(torch_tensor, epoch_int, index_int):
    ...
    """
    img_name = './normalized_our_data' + '/' + str(epoch_int)+ '_' + str(index_int) + '.png'
...
if __name__ == '__main__':
    ...
    for iteration in range(epochs):  # This will load the epochs.
        ...
        for index in tqdms:  # This will load the batches.
            ...
            img_re= model(img_normalized) # Send original image to the encoder and decoder model and get reconstructed image back.
            # print(img_re.size())
    ...
    scio.savemat('./train_result/loss_all_normalized_our_data.mat',{'grd_loss_all':grd_loss_all,'hist_loss_all':hist_loss_all,
