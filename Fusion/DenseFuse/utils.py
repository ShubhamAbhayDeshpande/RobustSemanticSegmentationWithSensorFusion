...
def gradient_gray(image):
    ...
    # Convert the numpy array into tensor and assign that tensor to the learnable weights of the convolution operator conv_op.
    conv_op.weight.data = (torch.from_numpy(kernel)).cuda().type(torch.float32)

    # Detecting edges (gradient) By applying above kernel to the input image.
    edge_detect = conv_op(image)
...
#     #edge_detect = conv_op(image.unsqueeze(0))
#     edge_detect = conv_op(image)

#     edge_detect = edge_detect.squeeze(0)
