...
parser.add_argument(
    "--config_file",
    type=str,
    default="inference.yml",
    help="Path to the folder containing inference config file",
    required=False,
)
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(file["output folder"], file["experiment name"])
    os.makedirs(output_dir, exist_ok=True)

    # Transforms for dataloader
    transform = transforms.Compose(
        [
            transforms.Resize(file["img size"]),
            transforms.ToTensor(),
    ...
    inference_data = InferenceDataset(
        folder_nir_path=file["NIR img folder"],
    ...
    # Load trained model
    vae_net = VAE(
        channel_in=inference_image["rgb_img"][0].shape[0], ch=128, latent_channels=256
    )
